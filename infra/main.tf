...
locals {
  ...
  security_groups_ids = data.aws_security_groups.secruity_groups.ids
...
# 인스턴스에 추가할 보안 그룹
# vpc_security_group_ids = data.aws_security_groups.secruity_groups.ids
data "aws_security_groups" "secruity_groups" {
  filter {
    name = "tag:Name"
    values = [
...
data "aws_subnets" "target_subnets" {
  filter {
    name   = "vpc-id"
    values = [data.aws_vpc.vpc.id] # 사용 중인 VPC ID 변수 지정
  }
  filter {
    name   = "tag:Type"
    values = ["private"]
...
# ALB 보안그룹
# data "aws_security_group" "secruity_group_alb" {
#   filter {
#     name   = "tag:Name"
...
# }
# # SSH 보안그룹
# data "aws_security_group" "secruity_group_ssh" {
#   filter {
#     name   = "tag:Name"
...
output "information" {
  value = [
    local.vpc_id,
    data.aws_security_groups.secruity_groups.ids,
    # data.aws_security_group.secruity_group_alb.id,
    # data.aws_security_group.secruity_group_ssh.id
  ]
}
...
resource "aws_iam_role" "codepipeline_role" {
  name = "${local.tag_header}AmazonCodePipelineService-Role" # 역할 이름

  # jsonencode 함수를 사용해 HCL Map 구조로 신뢰 관계 정책 (Assume Role Policy) 정의
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect    = "Allow"
...
# Pipeline 구성에 필요한 배포 파일 저장소 기본 생성
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        FullRepositoryId = "csjin21c/ex-aws-codepipeline"
        # 브랜치 정의
        BranchName = "main"
      }
    }
